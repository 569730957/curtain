...
<body>
	...
	<form:form id="inputForm" modelAttribute="tGoods" action="${ctx}/tgoods/tGoods/save" method="post" class="form-horizontal">
			...
			<div class="controls">
				<select name="fabric"  class="wid80">
					<c:forEach items="${listFabric }" var="f">
						<option value="${f.dicvalue }">${f.dicname}</option>
					</c:forEach>
				</select>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:forEach items="${listCate }" var="f">
					<input type="checkbox" name="cateid" value="${f.cateid }" />${f.catename }
				</c:forEach>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<input type="hidden" id="goodsdetail" name="goodsdetail" value="" />
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">排序：</label>
			<div class="controls">
				<form:input id="sort" path="sort" htmlEscape="false" maxlength="3" cssClass="wid80" class="input-xlarge number"/>
				(数字越小，商品显示越靠前)
			</div>
		</div>
